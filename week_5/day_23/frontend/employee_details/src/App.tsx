...
function App() {
  ...
  const { data: notificationData } = useQuery<NotificationItem | null>({
  ...
  useEffect(() => {
    const eventSource = new EventSource('http://127.0.0.1:8000/notifications/stream')
      ...
      <div className="mx-auto max-w-6xl rounded-2xl bg-white p-8 shadow-lg">
        <div className="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">Live Notifications</p>
              <h2 className="mt-1 text-lg font-semibold text-slate-800">
                {latestNotification?.title ?? 'No notification yet'}
              </h2>
              <p className="mt-1 text-sm text-slate-600">
                {latestNotification?.server_message ?? 'Post a notification from the server to see it here.'}
              </p>
            </div>
            <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-blue-700">
              {latestNotification?.not_type ?? 'info'}
            </span>
          </div>
        </div>
